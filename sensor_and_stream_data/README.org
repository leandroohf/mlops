...
* Phase 2: Scalable Ingestion Pipeline with Cloud Dataflow
 ...
 | features engineering | every 30 minutes   | < 15 minutes     | simple                  | new (not present in Phase 1)             |                                        |
 | ml training          | every 2 hour       | < 30 minutes     | one model for all data  | still out of scope                       |                                        |
 | ml predictions       | every 1 hour       | < 10 minutes     |                         | still out of scope                       | multiple runs due to changing features |
 * NOTEs:
...
** Setup the cloud dataflow (Apache beam)
    ...
    * env vars: We gonna use the same as before plus:
      ...
      #+begin_src sh
        ...
        JOB_NAME="bike-share-raw-events" # NOTE: Unique among active jobs in the same region
      #+end_src
...
* Phase 3: Scalable Pipeline
  ...
  | features engineering | every 25 minutes   | < 1h                    | complex                     | upgraded from simple -> complex + higher freq |                                            |
  ...
  * NOTEs:
  ...
  In Phase 3 we have the qreuirments that project grew up to different
  locations, cities and countries and the feature engineering are more complex
  and requireed to higher frequency than before (TODO define this parameters)
   ...
   cluster the tables by station_id and citties.
       ...
       * we decide to partiotining and cluster features table for good ingestion
        ...
        #+begin_src sh
          ...
          CREATE TABLE IF NOT EXISTS \`${RAW_EVENT_TABLE}\` (
            ...
            kind STRING,
            delta INT64,
            event_date DATE GENERATED ALWAYS AS (DATE(ts_iso)) STORED -- NOTE: STORED = materialized (not virtuak) column (required for partitioning)
          )
          ...
          CREATE TABLE IF NOT EXISTS \`${FEATURES_TABLE}\` (
            ...
            value FLOAT64,
            as_of TIMESTAMP,
            -- NOTE: STORED = materialized (not virtuak) column (required for partitioning)
            event_date DATE GENERATED ALWAYS AS (DATE(as_of)) STORED
          )
          PARTITION BY event_date
