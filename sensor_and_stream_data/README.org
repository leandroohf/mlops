...
* Project Introduction: Sensor Data Ingestion -> Features

  This project is a learning prototype that simulates *bike-station sensor
  events* (pickups/dropoffs) and builds a cloud-native pipeline on Google Cloud.

  The project is intentionally structured in *three phases* so we can start with
  the simplest working system (MVP), then progressively improve *scalability*
  and *operational robustness* as the requirements grow.

  The long-term goal is to support a full ML system (features → training →
  predictions), but in this project we focus mainly on:
  - ingesting sensor data reliably
  - storing raw events
  - computing features on a schedule
  - preparing the architecture for larger scale
...
*** Setup infrastructure
  ...
  * Creating table
    ...
    #+begin_src sh
      ...
      bq ls $PROJECT_ID:bike_streaming
      ...
      # delete table
      bq rm -f -t $PROJECT_ID:bike_streaming.events
...
** Phase 2: Scalable Ingestion Pipeline with Cloud Dataflow
  ...
  This phase upgrades the MVP (Phase 1) to support *high-volume streaming
  ingestion* using *Cloud Dataflow (Apache Beam)*, while keeping feature
  computation as a *scheduled job* using *Cloud Scheduler → HTTP Cloud Function →
  BigQuery upsert*.

  - Ingestion: near real-time, scalable, fault-tolerant
  - Feature Engineering: scheduled (e.g., every 30–60 min), simple aggregations
  - ML training/prediction: out of scope

  | what                 | frequency          | process duration | scope                                | changes                             | comment                                   |
  |----------------------+--------------------+------------------+--------------------------------------+-------------------------------------+-------------------------------------------|
  | data ingestion       | close to real time | < 5 minutes      | simple with high volume              | switched Cloud Function -> Dataflow | streaming job always listening to Pub/Sub |
  | features engineering | every 1 hour       | < 15 minutes     | simple. min window length 30 minutes | new in Phase 2                      | Cloud Scheduler triggers HTTP Cloud Fn    |
  | ml training          | every 2 hours      | < 30 minutes     | one model for all data               | out of scope                        |                                           |
  | ml predictions       | every 1 hour       | < 10 minutes     | synced with feature engineering      | out of scope                        |                                           |
  * NOTEs:
  ...
  * projects assets (BOM: Bil of materials)
   | assets                      | qty | comments                                                    |
   |-----------------------------+-----+-------------------------------------------------------------|
   | Pub/Sub topic               |   1 | input event stream (sensor events)                          |
   | Pub/Sub subscription        |   1 | Dataflow reads from subscription                            |
   | BigQuery dataset            |   1 | bike_streaming                                              |
   | BigQuery tables             |   2 | raw_events (ingested), station_features (computed features) |
   | GCS bucket                  |   1 | Dataflow staging/tmp + optional debug outputs               |
   | Dataflow job (streaming)    |   1 | Pub/Sub -> BigQuery raw_events                              |
   | Cloud Function (HTTP, gen2) |   1 | compute features + upsert into BigQuery station_features    |
   | Cloud Scheduler             |   1 | triggers Cloud Function periodically                        |
   | Service account(s)          |   1 | used by Dataflow + Scheduler/Function (or separate SAs)     |

   In Phase 2:

    1. We simulate a continuous/high-frequency sensor stream (sensors.py).
    2. Pub/Sub acts as the ingestion buffer.
    3. A *streaming Dataflow job* consumes Pub/Sub messages and writes raw events
       into BigQuery `raw_events`.
    4. A *Cloud Scheduler* triggers an *HTTP Cloud Function* periodically to compute
       simple aggregations and *upsert* features into BigQuery `station_features`.

    This design matches a common production pattern:
    * Dataflow handles scaling + reliability for streaming ingestion.
    * Feature computation is scheduled when features are only needed
      periodically and are cheap to compute.

     #+begin_src text
       sensors.py --> Pub/Sub Topic --> Dataflow (stream) --> BigQuery.raw_events
                                              |
                                              v
                              Cloud Scheduler --> Cloud Function (HTTP) --> BigQuery.station_features
       #+end_src

       #+begin_src markdown
       ```mermaid
       flowchart TD
         A[Sensor Script (sensors.py)] --> B[Pub/Sub Topic]
         B --> C[Dataflow Streaming Job<br/>Pub/Sub -> BigQuery raw_events]
         C --> D[BigQuery: raw_events]

         E[Cloud Scheduler<br/>every 30-60 min] --> F[Cloud Function (HTTP, gen2)<br/>compute + upsert features]
         F --> G[BigQuery: station_features]
      #+end_src

    * Notes / Clarifications

      - The ingestion path is fully streaming and scales via Dataflow.

      - Feature computation is scheduled (not streaming) because:
        - features are needed every N minutes, not continuously
        - computations are simple and cost-effective in BigQuery
      - ML training and prediction remain out of scope for this project phase.
...
** Phase 3: Scalable Pipeline
   ...
   * projects assets (BOM: Bil of materials)
   ...
   | cloud scheduler                      |   1 | optional: can be replaced by astronome/airflow if get complex |
   | service accounts                     |   1 |                                                               |
   ...
   * The feature get complex and need to be computed more often (features
   ...
   Implementation:
     ...
     * Features engineering
       ...
       + Initial version: Cloud Run Job triggered by Cloud Scheduler
       ...
       + setup cloud scheduler
         ...
         * set docker images
           ...
           #+begin_src sh
             ...
             # mac arch arm64 need to build the image for target arch: amd64 (x86_64)
             docker buildx build \
             ...
             # good idea to inspect the image and the arch it was built
             ...
             # NOTE: start dataflow job from runner docker container
         ...
         * set cloud job
           ...
           #+begin_src sh
             ...
             # 1.0) defined by the script: scripts/run_apache_beam_batch_raw_events_to_features.py
             gcloud run jobs create update-station-features-launcher \
               --image="${IMAGE}" \
               --region="${REGION}" \
               --project="${PROJECT_ID}" \
               --service-account="${SA}" \
               --set-env-vars="PROJECT_ID=${PROJECT_ID},REGION=${REGION},RAW_EVENT_TABLE=${RAW_EVENT_TABLE},FEATURES_TABLE=${FEATURES_TABLE},BUCKET=${BUCKET},SA=${SA},FEATURE_ENGINEERING_JOB_NAME=${FEATURE_ENGINEERING_JOB_NAME}"

             # 1.1.) list all jobs
             gcloud run jobs list --project="$PROJECT_ID" --region="$REGION"

             # 1.2) check the service account and permissions and docker registry
             ...
             gcloud projects get-iam-policy "$PROJECT_ID" \
                      --flatten="bindings[].members" \
                      --filter="bindings.members:serviceAccount:${SA} AND bindings.role:roles/artifactregistry" \
             ...
             # 1.4) inspect cloud jobs
             gcloud run jobs list --project="$PROJECT_ID" --region="$REGION"

             # inspect dataflow jobs
             gcloud dataflow jobs list --region="$REGION" --project="$PROJECT_ID"

             gcloud run jobs executions describe beamapp-root-1224230338-785453-hn2iadmz update-station-features-launcher-jdzqn \
                    --project="$PROJECT_ID" \
                    --region="$REGION" \
                    --format="value(status.conditions[0].type,status.conditions[0].status,status.completionTime,status.failedCount,status.succeededCount)"
           #+end_src

         * set the scheduler

           #+begin_src sh
             # NOTE: add permission to SA trigger jobs
             gcloud iam service-accounts list --project=mlops-project-abacabb
             gcloud projects get-iam-policy "${PROJECT_ID}" \
                    --flatten="bindings[].members" \
                    --filter="bindings.members:serviceAccount:${SA}" \
             ...
             # NOTE: triger with curl
             ENDPOINT="https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_ID}/jobs/update-station-features-launcher:run"
             ...
             # NOTE: schedule every N minutes
             # needs oauth not OIDC Token
             gcloud scheduler jobs create http update-station-features-scheduler \
                    --schedule="*/20 * * * *" \
                    --http-method=POST \
                    --uri="${ENDPOINT}" \
                    --time-zone="America/Los_Angeles" \
